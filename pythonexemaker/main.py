...
def upload_to_supabase(system_info):
    """Upload system information to Supabase, completely replacing existing record if it exists"""
    try:
        ...
        if response.data and len(response.data) > 0:
            try:
                # Delete the existing record completely
                supabase.table("device_specs").delete().eq("device_id", device_id).execute()
                print(f"Deleted existing record for device: {device_id}")
            except Exception as delete_error:
                print(f"Error deleting record: {delete_error}")
                return None
        
        # Now insert the new record (whether there was an existing one or not)
        try:
            insert_response = supabase.table("device_specs").insert(system_info).execute()
            print(f"Inserted new record for device: {device_id}")
        ...
        print(f"Error in Supabase operation: {e}")
        return None
